...
const openModalForm = (op, a) => {
    ...
    form.clearErrors();
    if (op === 1) {
        title.value = 'Create Beneficiary';
    } else {
        title.value = 'Edit Beneficiary';
        form.name = a.name;
        form.email = a.email;
        form.password = '';
...
// Usamos SweetAlert para confirmar eliminaciones
const confirmDelete = (beneficiario) => {
    Swal.fire({
        title: 'Are you sure?',
        text: `You won't be able to revert this! Deleting Benefiaciary: ${beneficiario.name}`,
        icon: 'warning',
...
// Función para guardar (crear o editar) con alertas de éxito
const save = () => {
    if (!form.password) {
    ...
    if (operation.value === 1) {
        form.post(route('admin-beneficiaries.store'), {
            ...
            onError: () => {
                Swal.fire('Error', 'There was an error creating the beneficiary.', 'error');
            }
        });
    } else {
        form.put(route('admin-beneficiaries.update', v.value.id), {
            ...
            onError: () => {
                Swal.fire('Error', 'There was an error updating the beneficiary.', 'error');
            }
        });
    }
}
