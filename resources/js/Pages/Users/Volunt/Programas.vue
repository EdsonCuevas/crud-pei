...
// Inicialización de un formulario utilizando Inertia.js
const form = useForm({
    name:'',
    email:'',
    phone:'',
...
// Objeto reactivo para manejar los datos de un voluntario
const v = ref({
    id:'',
    name:'',
    phone:'',
...
// Funciónes  para abrir los modales 
...
const save = () => {
    if (operation.value === 1) {
        form.post(route('volunt.store'), {
            onSuccess: () => ok('Voluntario Creado'),
        });
    } else {
        form.put(route('volunt.update', v.value.id), {
